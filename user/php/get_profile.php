...
try {
    // Fetch user info from users table (no need for user_profiles table)
    $stmt = $conn->prepare("
    ...
    // Fetch stats from your existing tables
    $stats = [];
    ...
    // Unclaimed orders count (adjust based on your business logic)
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM orders WHERE user_id = ? AND status = 'unclaimed'");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
